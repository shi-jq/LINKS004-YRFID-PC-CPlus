...
namespace Szaat.RFID.CSharpAPI
{
    public class RfidApi
    {
        ...
        [DllImport("RFIDAPI.dll", EntryPoint = "SAAT_YRevIDMsgDecRssiExpand")]
        public static extern int SAAT_YRevIDMsgDecRssiExpand(IntPtr pHandle, out byte nTagType, out uint pId, out int nRssi, out int nAntenna,out int nParam1, out int nParam2);

        [DllImport("RFIDAPI.dll", EntryPoint = "SAAT_YTagSelect")]
        public static extern bool SAAT_YTagSelect(IntPtr pHandle, byte nOpEnable, byte nMatchType, byte[] MatchData, byte nLenth);
    }
}